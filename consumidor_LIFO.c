...
void consumidor(void){
    ...
    sleep(1); //Introducimos un sleep para esperar a que el productor llene la cola luego de que el consumidor haya hecho lo mismo

    unsigned int prioridad;
    for(i=0; i < DATOS_A_CONSUMIR; i++){
        mq_receive(almacen2,&mensaje,sizeof(char),&prioridad);
        printf("\n(C): Se ha recibido --%c--\n", mensaje);
        mq_send(almacen1,&mensaje,sizeof(char),0);
        printf("(C): Se ha consumido --%c--\n", mensaje);
    }

    printf("Se han consumido todos los datos\n");
    for(i=0;i<MAX_BUFFER;i++){
        mq_receive(almacen1,&mensaje,sizeof(char),&prioridad);
    }
